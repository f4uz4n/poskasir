...
@section('heading', 'Piutang')
@section('subheading', 'Tagihan pelanggan yang belum lunas')

@section('content')
<div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
    <div class="card p-5">
        <div class="text-sm text-slate-500">Total piutang</div>
        <div class="text-2xl font-extrabold mt-1">Rp {{ number_format($summary['total'], 0, ',', '.') }}</div>
    </div>
    <div class="card p-5">
        <div class="text-sm text-slate-500">Sudah diterima</div>
    ...
    <div class="card p-5">
        ...
        <div class="text-2xl font-extrabold mt-1 text-amber-600">Rp {{ number_format($summary['outstanding'], 0, ',', '.') }}</div>
    </div>
    <div class="card p-5">
...
<div class="grid lg:grid-cols-3 gap-4 mb-6">
    <form method="POST" action="{{ route('receivables.store') }}" class="card p-5 space-y-3 lg:col-span-1">
        @csrf
        <h2 class="font-bold">Tambah piutang manual</h2>
        <input name="party_name" class="input" placeholder="Nama pelanggan" required>
        <input type="number" min="1" name="amount" class="input" placeholder="Jumlah" required>
        <input type="date" name="due_date" class="input" placeholder="Jatuh tempo">
    ...
    <div class="card p-4 sm:p-5 lg:col-span-2">
        <form method="GET" class="flex flex-wrap gap-2 mb-4">
            <input type="text" name="q" value="{{ request('q') }}" class="input" placeholder="Cari kode / pelanggan">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm min-w-[700px]">
                ...
                <tbody>
                        ...
                        <tr class="border-b border-slate-100 align-top">
                            ...
                            <td class="py-3">
                                ...
                                <div class="text-xs text-slate-400">
                                    {{ $item->source }}
                                    @if($item->transaction) · {{ $item->transaction->invoice_number }} @endif
                            ...
                            <td class="py-3">
                                @if($item->status !== 'paid')
                                <form method="POST" action="{{ route('receivables.pay', $item) }}" class="flex flex-wrap gap-1 items-center">
                                    @csrf
                                    ...
                                    <button class="btn btn-secondary text-xs px-2">Terima</button>
                                </form>
                                @else
                    ...
                    @empty
                        <tr><td colspan="5" class="py-10 text-center text-slate-500">Belum ada piutang.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
